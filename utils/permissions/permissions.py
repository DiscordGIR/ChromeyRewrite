...
class Permissions:
    ...
    def __init__(self):
        ...
        self._role_permission_mapping = {
            1: the_guild.role_nerds,
            2: the_guild.role_moderator,
            3: the_guild.role_administrator,
        }
        ...
        self._permissions = {
            ...
            4: (lambda guild, m: self.has(guild, m, 9) or (guild.id == cfg.guild_id
                and m == guild.owner)),
    ...
    def level_role_list(self, level: int) -> List[int]:
        if level == 0:
            ...
            return [Permission(id=cfg.owner_id, type=2, permission=True)] + [ Permission(id=cfg.guild_owner_id, type=2, permission=True)]
        ...
        return [Permission(id=self._role_permission_mapping[_level], type=1, permission=True) for _level in range(level, 4)] \
            + [Permission(id=cfg.owner_id, type=2, permission=True) ] + [ Permission(id=cfg.guild_owner_id, type=2, permission=True)]  # bot owner permission
